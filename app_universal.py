...
def analise_basica(df):
    ...
    with col3:
        percent_completo = (1 - df.isnull().sum().sum() /
...
def analise_cruzada(df, tipos):
    ...
    if not colunas_numericas or not colunas_categoricas:
        st.warning(
            "⚠️ É necessário ter pelo menos uma coluna numérica e uma categórica para análise cruzada.")
        return
...
def insights_automaticos(df, tipos):
    ...
    if len(colunas_nulos) > 0:
        insights.append(
            f"⚠️ **Atenção**: {len(colunas_nulos)} coluna(s) com mais de 50% de valores nulos: {', '.join(colunas_nulos[:3])}")

    # Colunas com valores únicos (possíveis IDs)
    for col in df.columns:
        if df[col].nunique() == len(df):
            insights.append(
                f"🔑 **{col}**: Possui valores únicos - possível identificador único")
    ...
    for col in tipos['numericas'][:5]:  # Limitar a 5 colunas
        ...
        if outliers > 0:
            insights.append(
                f"📊 **{col}**: Detectados {outliers} outliers ({outliers/len(df)*100:.1f}%)")
    ...
    for col in tipos['categoricas'][:5]:
        ...
        if len(valor_counts) > 1:
            ...
            if proporcao_maior > 0.8:
                insights.append(
                    f"⚖️ **{col}**: Desbalanceada - {valor_counts.index[0]} representa {proporcao_maior*100:.1f}% dos dados")
        ...
        for i in range(len(correlacao.columns)):
            for j in range(i+1, len(correlacao.columns)):
                if abs(correlacao.iloc[i, j]) > 0.7:
                    insights.append(
                        f"🔗 **Correlação forte** ({correlacao.iloc[i, j]:.2f}) entre {correlacao.columns[i]} e {correlacao.columns[j]}")
    ...
    if insights:
        ...
        st.info(
            "✅ Nenhum insight crítico detectado. Os dados parecem estar em boa forma!")
...
def main():
    ...
    # Cabeçalho
    st.markdown('<h1 class="main-header">📊 Analisador Universal de Dados CSV</h1>',
                unsafe_allow_html=True)
    st.markdown(
        "**Faça upload de qualquer arquivo CSV e obtenha análises automáticas instantâneas!**")
    st.markdown("---")
    ...
    if tipos['categoricas']:
        ...
        if col_filtro != "Nenhum":
            ...
            df_filtrado = df_filtrado[df_filtrado[col_filtro].isin(
                valores_selecionados)]
            st.sidebar.markdown(
                f"**Registros após filtro:** {len(df_filtrado):,}")
